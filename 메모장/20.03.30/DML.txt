+ INDERT - DML
  ...
  - INSERT 구문 형식
      ...
      -- 값의 순서는 COLUMN_ID를 따른다
      INSERT INTO tablename
      VALUES ( data1, data2, data3, ... );
      
      
      -- values 절 대신 select구문을 이용하여 조회된 값을 삽입
      INSERT INTO tablename (col1, col2, col3, ...)
      SELECT 절;
...
+ MERGE - DML
  ...
  - MERGE 구문 형식
        ...
        -> ON절 : WHEN절에서 반응하는 조건을 부여하는 절
        
        -> WHEN MATCHED
                ON절의 조건에 만족하는 USING절 테이블의 데이터가 존재하면 수행한다
                UPDATE, DELETE를 수행
        
        -> WHEN NOT MATCHED
                ON절의 조건에 만족하는 USING절 테이블의 데이터가 없으면 수행한다
                INSERT 를 수행        
...
+ 트랜잭션, Transaction 
    ...
    - 클라이언트마다 데이터베이스에 작업한 내역들을 트랜잭셕이라고 한다.
